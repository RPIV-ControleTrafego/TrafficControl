...
public interface AdminRepo extends CrudRepository<Admin, Integer> {
   ...
   */
  @Query(
    value = "select CASE WHEN count(1) > 0 THEN 'true' ELSE 'false' END  from admins where id = :id",
    nativeQuery = true
  )
  public boolean exist(int id);

  @Query(
    value = "select * from admins where email = :email and senha = :senha",
    nativeQuery = true
  )
  public Admin loginCheck(String email, String senha);
